...
def decision_tree_depth_test(x_train, y_train, x_test, y_test):
    ...
    print("Best accuracy = ", float(best_acc), ", at depth = ", best_depth)
...
def adaboost_l_test(x_train, y_train, x_test, y_test):
    depth_range = range(10, 200, 10)
    accuracy_tr = []
    accuracy_ts = []
    f1_s = []
    best_l, best_acc = 0, 0
    ...
    ax.legend(loc="lower right")
    plt.xlabel('L')
    plt.title("Adaboost behavior across L values")
    plt.show()

    print("Best accuracy = ", float(best_acc), ", with L = ", best_l)
